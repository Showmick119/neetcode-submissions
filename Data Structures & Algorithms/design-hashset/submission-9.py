...
class MyHashSet:
    ...
    def _resize(self) -> None:
        ...
        for key in temp:
            ...
            # if it's 'DELETED' or None, you don't add it to the new
            # array. we already re-initialize with None, we only want
            # to reinsert actual valid keys into the new table, not
            # None keys.

            # 'DELETED' is a signal that it shouldn't be in the Hashset
            # anymore, so don't bother processing it. Only valid keys.
            # 'DELETED' is only kept such that we don't hurt other
            # key's probing chains.
