class Solution:
    def containsNearbyDuplicate(self, nums: List[int], k: int) -> bool:
        ...
        for num in nums:
            if len(w) >= k + 1:
                w.popleft() # if its at max window length of k + 1, pop an element such that
                # we have room for the next element which we are adding in this loop. upon
                # adding that next element, we still maintain that max window length of k + 1.
            if num in w:
                return True
            w.append(num)
        return False
