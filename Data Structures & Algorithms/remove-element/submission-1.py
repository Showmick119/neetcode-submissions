    ...
    def removeElement(self, nums: List[int], val: int) -> int:
        ...
        while (i < len(nums) and k <= i):
            if nums[i] != val:
                ...
                count += 1
                i += 1
                k += 1 # only increment k, after we have processed
                # that specific k-index with an array element not
                # equal to val. If it equals to val, we ignore it,
                # and in the process implicitly remove it.
            else:
                i += 1
        return count
