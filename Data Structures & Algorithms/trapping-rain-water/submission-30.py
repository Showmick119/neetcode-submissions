class Solution:
    def trap(self, height: List[int]) -> int:
        ...
        while l <= r:
            ...
            curr = height[l]
            areaHeight = min(maxLeft, maxRight)
            print(f'maxLeft = {maxLeft}, maxRight = {maxRight}, curr = {curr}, areaHeight = {areaHeight}')
            maxArea += max(0, areaHeight - curr)
            if maxLeft > maxRight:
                if r != l and r < len(height) - 1:
